...
void ShortTermMemory::WriteToDisk(std::ofstream* s) {
  ...
  Serialize(s, match_byte_context);
  Serialize(s, match_bigram_context);
  Serialize(s, error_context);
  Serialize(s, error_history);
  Serialize(s, failcount_context);
  Serialize(s, error_byte_context);
  Serialize(s, direct_two_bytes);
  Serialize(s, prev_two_bytes);
...
void ShortTermMemory::Copy(const MemoryInterface* m) {
  ...
  match_byte_context = orig->match_byte_context;
  match_bigram_context = orig->match_bigram_context;
  error_context = orig->error_context;
  error_history = orig->error_history;
  failcount_context = orig->failcount_context;
  error_byte_context = orig->error_byte_context;
  direct_two_bytes = orig->direct_two_bytes;
  prev_two_bytes = orig->prev_two_bytes;
