...
void Predictor::AddDirect() {
  ...
  Direct* direct1 =
      new Direct(short_term_memory_, 30, short_term_memory_.last_byte_context,
                 256, long_term_memory_.direct_1);
...
void Predictor::AddMixers() {
  Mixer* mixer1 =
      new Mixer(short_term_memory_, short_term_memory_.last_byte_context,
                short_term_memory_.predictions, 0.005, false);
  models_.push_back(std::unique_ptr<Model>(mixer1));
  Mixer* mixer2 = new Mixer(short_term_memory_, short_term_memory_.always_zero,
                            short_term_memory_.predictions, 0.005, false);
  models_.push_back(std::unique_ptr<Model>(mixer2));
  Mixer* mixer3 =
      new Mixer(short_term_memory_, short_term_memory_.last_two_bytes_context,
                short_term_memory_.predictions, 0.005, false);
  models_.push_back(std::unique_ptr<Model>(mixer3));
  Mixer* second_layer =
      new Mixer(short_term_memory_, short_term_memory_.always_zero,
