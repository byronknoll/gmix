...
void EncodeByte(unsigned char c, FILE* output) {
  if (c == kEndUpper || c == kEscape || c == kUppercase || c == kCapitalized ||
      c == kQuote || c >= 0x80) {
    putc(kEscape, output);
  }
  putc(c, output);
...
void Dictionary::EncodeWord(const std::string& word, int num_upper,
  ...
  if (num_upper > 1)
    putc(kUppercase, output);
  else if (num_upper == 1)
    putc(kCapitalized, output);
...
void Dictionary::Decode(FILE* input, FILE* output) {
  unsigned char c;
  while (DecodeChar(input, c)) {
    putc(c, output);
  }
}

bool Dictionary::DecodeChar(FILE* input, unsigned char& output) {
  while (output_buffer_.empty()) {
    if (!AddToBuffer(input)) return false;
  }
  output = output_buffer_.front();
  output_buffer_.pop_front();
  return true;
}

bool Dictionary::AddToBuffer(FILE* input) {
  int next = getc(input);
