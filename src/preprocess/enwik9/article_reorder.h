...
enum ParserState { expect_page = 0, expect_id, expect_pageend };
...
void loadFile(const char *fname) {
  ...
  while (wfgets(s, 8192 * 8, file)) {
    ...
    if (strstr(s, "infobox ") || strstr(s, "infobox_") ||
        strstr(s, "Infobox_") || strstr(s, "Infobox "))
      acc.infobox = s, acc.infobox.pop_back();
    if (strstr(s, "#REDIRECT") || strstr(s, "#redirect") ||
        strstr(s, "softredirect") || strstr(s, "#Redirect") ||
        strstr(s, "#REdirect"))
      acc.redirect = s, acc.redirect.pop_back();
#endif
...
void reorder() {
  ...
  std::ifstream infile2("enwik9_article_order");
  ...
  for (int i = 0; i < positions.size(); i++) {
    int pos = positions[i];
#ifdef DUMPARTICLE
    printf("%d\t%d\t%d\t%d\t%s\t%s\t%s\n", pos, vec[pos].id, vec[pos].start,
           vec[pos].end, vec[pos].title.c_str(), vec[pos].infobox.c_str(),
           vec[pos].redirect.c_str());
#else
